...
"""
def do_job(i,file_list_all):      #处理函数  处理第i个模型
    #根据id号，截取对应的目标模型名称
    ...
    worker(i, 100, 20, good_grasp, file_list_all)

    #对CAD模型进行Antipod采样并筛选得到的候选抓取
    home_dir = os.environ["PointNetGPD_FOLDER"]
    good_grasp_file_name = os.path.join(
        home_dir, "dex-net", "apps", "generated_grasps",
        "{}_{}_{}".format(filename_prefix, str(object_name), str(len(good_grasp))))
...
def worker(i, sample_nums, grasp_amount, good_grasp, file_list_all):  #主要是抓取采样器以及打分    100  20
    """
    ...
    config_path = os.path.join(home_dir, 'dex-net', 'test', 'config.yaml')
    yaml_config = YamlConfig(config_path)
    #设置夹名称
    gripper_name = "robotiq_85"
    #根据设置的夹爪名称加载夹爪配置
    gripper = RobotGripper.load(gripper_name, os.path.join(home_dir, "dex-net", "data", "grippers"))
    #设置抓取采样的方法
...
if __name__ == '__main__':
    ...
    home_dir = os.environ["PointNetGPD_FOLDER"]
    #存放CAD模型的文件夹
    file_dir = os.path.join(home_dir, "PointNetGPD", "data", "ycb-tools", "models", "ycb")   #获取模型的路径
    ...
    for _ in range(pool_size):   #想多线程处理多个模型，但是实际上本代码每次只处理一个
        ...
        pool.append(multiprocessing.Process(target=do_job, args=(job_i,file_list_all)))  #在pool末尾添加元素
    ...
    while len(job_list) > 0:    #如果有些没处理完
        for ind, p in enumerate(pool):
            if not p.is_alive():
                ...
                p = multiprocessing.Process(target=do_job, args=(job_i,file_list_all))
                p.start()
                pool.append(p)
                break
    print('All job done.')
