...
def get_optimizer(model, optimizer, args=None):
    ...
    if optimizer == "sgd":
        ...
        _lr = 5e-3 if "lr" not in args.keys() else args["lr"]
...
def get_args(parser):
    parser.add_argument(
        "--client_num_in_total", type=int, default=5, help="total num of clients",
    )
    parser.add_argument(
    ...
    parser.add_argument("--epochs", type=int, default=10, help="communication round")
    parser.add_argument(
        "--inner_loops", type=int, default=5, help="local epochs in FedAvg section"
    )
    parser.add_argument(
        "--server_lr",
        type=float,
