...
class BaseGraspDataset(torch.utils.data.Dataset):
    def __init__(self):
        ...
        self.transform = pickle.load(open(rf"{self.pointnetgpd_dir}\PointNetGPD\data\google2cloud.pkl", "rb"))
...
class PointGraspDataset(BaseGraspDataset):
    def __init__(self, obj_points_num, grasp_points_num, pc_file_used_num, grasp_amount_per_file, thresh_good,
        ...
        fl_grasp = glob.glob(rf"{self.pointnetgpd_dir}\PointNetGPD\data\ycb_grasp\{self.tag}\*.npy")
        ...
        for i in fl_pc:
            #k = i.split('/')[-4]
            k = os.path.split(os.path.dirname(i))[0].split(os.path.sep)[-2]
        ...
        for i in fl_grasp:
            #k = i.split('/')[-1].split('.')[0]
            k = os.path.splitext(os.path.basename(i))[0]
            self.d_grasp[k] = i
...
class PointGraspOneViewDataset(BaseGraspDataset):
    def __init__(self, grasp_points_num, grasp_amount_per_file, thresh_good,
                 thresh_bad, tag, with_obj=False, projection=False, project_chann=3, project_size=60, client_id: int = None):
        super().__init__()
        self.grasp_points_num = grasp_points_num
        self.grasp_amount_per_file = grasp_amount_per_file
        self.tag = tag
